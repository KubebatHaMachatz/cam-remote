...
| Nothing seems to happen on the device | `adb logcat -s cam-remote-app:CamRemote` shows every command as it arrives and its outcome, including the file a capture wrote. If nothing appears, the request never reached the agent. `adb logcat \| grep cam-remote-app` widens that to everything the app logged, not just command traffic. |
